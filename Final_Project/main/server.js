...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        ...
        console.log('Users count:', onlineUsers);
        try {
    ...
    socket.on('submitSelections', (data) => {
        ...
        if (Object.values(submissions).length+1 === maxUsers) {
...
server.listen(3000, '0.0.0.0', () => {
    ...
    fs.writeFile(path.join(dataDir, 'guesses.json'), '{}', (err) => {
        if (err) {
            console.error('Error initializing submissions.json:', err);
        } else {
            console.log('guesses.json initialized successfully.');
        }
    });
});
